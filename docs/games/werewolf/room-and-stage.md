...
The public day timer drives one short tick sample at 5, 4, 3, 2, and 1 seconds, then one gong when it reaches zero. The client records emitted cues across pause/resume, clears them on timer reset, and does not replay missed cues after a hidden tab becomes visible. No per-second server messages are required.
