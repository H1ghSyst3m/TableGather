...
- `TABLEGATHER_ADMIN_TOKEN` enables the protected admin room overview API and `/admin` dashboard. Open `/admin?token=<token>` in the browser; the app sends the token as a bearer token to the room server.
