...
describe("room websocket server", () => {
  ...
  it("uses forwarding headers from trusted proxy peers for room lookup limits", async () => {
    const url = await startServer(new RoomManager(), { trustedProxies: ["127.0.0.1"] });
    const first = await openSocket(url, { "x-forwarded-for": "203.0.113.20" });
    await exhaustRoomLookups(first, "trusted-first");

    const sameForwardedClient = await openSocket(url, { "x-forwarded-for": "203.0.113.20" });
    ...
    const differentForwardedClient = await openSocket(url, { "x-forwarded-for": "203.0.113.21" });
